...
function QuitzItem(props) {
    ...
    console.log(keys);
    const {name, img} = item(items, keys);

    const handleClick = () => {
        console.log(ans.current.value);
        if (items.data[ans.current.value].name === name) {
            props.changeOutput(oldArray => [[items.data[ans.current.value].name, img, name, ans.current.value, true], ...oldArray]);
        } else {
            props.changeOutput(oldArray => [[items.data[ans.current.value].name, img, name, ans.current.value, false], ...oldArray]);
        }
    }

    return (
        <div className={"item_quitz_select"}>
            <img src={img} className={"image_item_quitz_select"} alt={name}/>
